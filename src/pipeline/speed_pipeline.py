...
class SpeedPipeline:
    ...
    def reset_stream_state(self) -> None:
        """Reset ctx for step() mode (e.g., when RTSP reconnects)."""
        self._ctx = self._init_ctx()
    ...
    def _process_one(self, r, ctx: Dict[str, Any], fps: float, W: int, H: int):
        """
        Process one ultralytics Results 'r'.
        Returns: (vis_frame, json_obj)
        """
        ctx["global_idx"] += 1
        ...
        # ctx aliases
        static_cnt: Dict[int, int] = ctx["static_cnt"]
        moving_cnt: Dict[int, int] = ctx["moving_cnt"]
        state: Dict[int, str] = ctx["state"]

        mpp_ema: Dict[int, float] = ctx["mpp_ema"]
        speed_ema_fixed: Dict[int, float] = ctx["speed_ema_fixed"]
        speed_ema_ws: Dict[int, float] = ctx["speed_ema_ws"]
        speed_active: Dict[int, float] = ctx["speed_active"]
        debug_rows: List[Dict[str, Any]] = ctx["debug_rows"]

        last_boxes = ctx["last_boxes"]
        last_gray = ctx["last_gray"]
        last_idx = ctx["last_idx"]
        last_H = ctx["last_H"]

        orb = ctx["orb"]

        homo_inlier_ratio: Optional[float] = ctx["homo_inlier_ratio"]
        homo_inlier_count: Optional[int] = ctx["homo_inlier_count"]
                ...
                for (b, tid, c) in zip(boxes, ids, clses):
                    ...
                    if (not self.SHOW_MOVING_ONLY) or (state.get(tid) != "static"):
                        ...
                        if v_mps <= self.MAX_MPS_CLAMP:
                            ...
                            # v_old_kmh: previous ACTIVE smooth
                            ...
                            if self.ws_smoother is not None and v_ws_ema_kmh is not None:
                                speed_active[tid] = float(speed_ema_ws[tid])
                            else:
                                speed_active[tid] = float(speed_ema_fixed[tid])
        ...
        # update ctx back
        ctx["last_boxes"] = last_boxes
        ctx["last_gray"] = last_gray
        ctx["last_idx"] = last_idx
        ctx["last_H"] = last_H
        ctx["homo_inlier_ratio"] = homo_inlier_ratio
        ctx["homo_inlier_count"] = homo_inlier_count
        ...
        for (x1, y1, x2, y2), tid, c in zip(boxes, ids, clses):
            ...
            tracks.append({
                ...
                "state": str(state.get(tid, "unknown")),
                "speed_kmh": float(speed_active[tid] * 3.6) if tid in speed_active else None,
            })

        json_obj = {"frame": int(global_idx), "tracks": tracks}

        return vis, json_obj

    def step(self, frame: np.ndarray, fps_hint: float = 30.0):
        """
        Streaming single-frame API for RTSP bench.
        Returns (vis_frame, json_obj).
        NOTE: dt uses fps_hint and frame index gap; for real RTSP you can pass target_fps.
        """
        ...
        vis, js = self._process_one(r, self._ctx, fps=float(fps_hint), W=int(W), H=int(H))
        return vis, js
    ...
    def run(self, video_path: str, out_path: str) -> None:
        ...
        # local ctx for run() (do NOT reuse self._ctx)
        ctx = self._init_ctx()

        for r in stream:
            vis, _ = self._process_one(r, ctx, fps=float(fps), W=W, H=H)
            vw.write(vis)

        vw.release()

        # ---------- Export diagnostics ----------
        debug_rows = ctx["debug_rows"]
